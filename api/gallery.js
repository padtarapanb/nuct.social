...
function buildThumbUrl(secureUrl) {
  ...
  // g_auto ให้ Cloudinary เลือกจุดโฟกัส (คน/วัตถุเด่นในภาพ) เองแทนการครอปกลางภาพเสมอ
  // ช่วยแก้ปัญหารูปถูกครอปโดนหัวขาด/พลาดจุดสำคัญเวลาไม่ใช่รูปสี่เหลี่ยมจัตุรัสอยู่แล้ว
  return secureUrl.replace("/upload/", "/upload/w_500,h_500,c_fill,g_auto,f_auto,q_auto/");
}
...
export default async function handler(req, res) {
  ...
  // สำคัญ: บัญชี Cloudinary รุ่นใหม่ (Dynamic Folder Mode — แบบที่เห็นใน Media Library ตอนนี้)
  // จะ "ไม่" เก็บโฟลเดอร์ไว้ใน public_id เหมือนบัญชีรุ่นเก่าอีกต่อไป ทำให้ค้นด้วย `folder:` แบบเดิม
  // หาเจอบ้างไม่เจอบ้าง (นี่คือสาเหตุที่พิมพ์ชื่อโฟลเดอร์ถูกแล้วแต่ดึงรูปไม่ขึ้น) ต้องค้นด้วย field
  // `asset_folder` แทน — ใส่ค้นทั้งสองแบบพร้อมกันไว้เผื่อบัญชีเก่า/ใหม่ทั้งคู่ และครอบคลุมโฟลเดอร์ย่อยด้วย
  const escapedFolder = folder.replace(/"/g, '\\"');
  const expression = [
    `folder:${folder}/*`,
    `folder:"${escapedFolder}"`,
    `asset_folder="${escapedFolder}"`,
    `asset_folder:${folder}/*`,
  ].join(" OR ");
