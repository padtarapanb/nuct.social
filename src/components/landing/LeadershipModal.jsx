...
export default function LeadershipModal({ open, onClose, team, shepherding: shepherdingGroups = [] }) {
  const [tab, setTab] = useState("committee");
  const shepherding = useMemo(() => flattenTree(buildTree(shepherdingGroups)), [shepherdingGroups]);
      ...
      <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-2xl max-h-[85vh] overflow-hidden flex flex-col">
        ...
        <div className="p-6 overflow-y-auto">
          {tab === "committee" ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-4 gap-y-8">
              {shepherding.map((node, i) => (
                <LeadershipCard key={node.id ?? `${node.name}-${i}`} node={node} />
              ))}
            </div>
          ) : (
            <p className="text-sm text-gray-400 text-center py-10">ยังไม่มีข้อมูลโครงสร้างการอภิบาล</p>
          )}
        </div>
      </div>
    </div>
