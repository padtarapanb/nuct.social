...
export default function Gallery() {
  ...
  const galleryCategories = cloudinaryImages.length
    ...
    : (content.galleryAlbums || [])
        .filter((album) => album.is_active !== false)
        .map((album) => ({
          id: album.id,
          title: album.title,
          // รูปปก (cover_image) ถ้าไม่มีให้ใช้รูปแรกในอัลบั้มแทน
          image: album.cover_image || album.images?.[0] || "",
          fullImage: album.cover_image || album.images?.[0] || "",
        }));
