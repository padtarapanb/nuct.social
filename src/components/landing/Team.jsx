  ...
  return (
    <div ref={ref} className={`reveal reveal-delay-${(index % 4) + 1} text-center group`}>
      <div className="relative w-32 h-32 sm:w-36 sm:h-36 mx-auto mb-4">
        {hasRealPhoto ? (
          <img src={normalizeImageUrl(person.photo)} alt={person.name} className="w-full h-full rounded-[1.75rem] object-cover shadow-lg" />
        ) : (
          <div
            className="w-full h-full rounded-[1.75rem] flex items-center justify-center text-white font-display font-bold text-2xl shadow-lg transition-transform duration-500 group-hover:scale-105"
            style={{ background: "linear-gradient(135deg,#7C3AED,#EC4899,#F59E0B)" }}
          >
            {initials}
          </div>
        )}
        <div className="absolute inset-0 rounded-[1.75rem] flex items-center justify-center gap-3 bg-black/45 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
...
function LeadershipCard({ node, index }) {
  ...
  return (
    <div ref={ref} className={`reveal reveal-delay-${(index % 4) + 1} text-center group`}>
      <div className="relative w-32 h-32 sm:w-36 sm:h-36 mx-auto mb-4">
        {hasRealPhoto ? (
          <img src={normalizeImageUrl(node.photo)} alt={node.name} className="w-full h-full rounded-[1.75rem] object-cover shadow-lg" />
        ) : (
          <div
            className="w-full h-full rounded-[1.75rem] flex items-center justify-center text-white font-display font-bold text-2xl shadow-lg transition-transform duration-500 group-hover:scale-105"
            style={{ background: "linear-gradient(135deg,#7C3AED,#EC4899,#F59E0B)" }}
