...
function isDuplicateName(name, rows, excludeId) {
...
export default function AlbumEditor() {
  ...
  const [newAlbum, setNewAlbum] = useState({ name: "", images: [] });
  ...
  const handleAdd = async () => {
    setError("");
    const name = newAlbum.name.trim();
    if (!name) {
      setError("กรุณาใส่ชื่ออัลบั้ม");
      return;
    }
    if (isDuplicateName(name, rows)) {
    ...
    try {
      await createRow("galleryAlbums", {
        name,
        slug: slugify(name),
        folder: folderFromName(name),
        images: newAlbum.images,
      });
      setNewAlbum({ name: "", images: [] });
      await load();
    } catch (err) {
      console.error(err);
  ...
  const handleSave = async (id) => {
    ...
    if (isDuplicateName(name, rows, id)) {
      setError("มีอัลบั้มนี้อยู่แล้ว");
      return;
    }
    setSavingId(id);
    ...
    <div>
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 mt-1">
          พิมพ์แค่ชื่ออัลบั้ม ระบบจะสร้างโฟลเดอร์ให้ตรงกับ Cloudinary ({ROOT_FOLDER}/ชื่ออัลบั้ม) ให้เอง
          จากนั้นวางลิงก์รูปที่อัปโหลดไว้ใน Cloudinary ทีละรูปได้เลย (ยังไม่ได้ผูกกับหน้า Gallery หลักบนเว็บไซต์)
        </p>
      </div>
          ...
          <div>
            ...
            {newAlbum.name.trim() && (
              <p className="text-xs text-gray-400 mt-1">
                folder: {folderFromName(newAlbum.name)} · slug: {slugify(newAlbum.name)}
              </p>
            )}
          </div>
          <ImagesField
              ...
              <div key={row.id} className="bg-white border border-gray-200 rounded-2xl p-5">
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    />
                    <p className="text-xs text-gray-400 mt-1">
                      folder: {folderFromName(draft.name)} · slug: {slugify(draft.name)}
                    </p>
                  </div>
                  <ImagesField
                    images={draft.images || []}
