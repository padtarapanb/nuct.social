...
export default function TreeEditor() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setFlatRows(rows);
      const d = {};
      rows.forEach((r) => (d[r.id] = { name: r.name || "", photo: r.photo || "" }));
      setDrafts(d);
    } catch (err) {
      console.error(err);
  ...
  const handleSave = async (id) => {
    ...
    try {
      ...
      await updateShepherdingNode(id, { name: draft.name ?? "", photo: draft.photo ?? "" });
      await load();
    } catch (err) {
  ...
  return (
    <div style={{ marginLeft: depth === 0 ? 0 : 22 }} className="mt-2">
      <div className="flex items-center gap-2 bg-white border border-gray-200 rounded-xl px-3 py-2.5 flex-wrap">
        ...
        {(() => {
          const photo = drafts[node.id]?.photo || "";
          const hasPhoto = /^https?:\/\//.test(photo);
          return hasPhoto ? (
            <img src={photo} alt="" className="shrink-0 w-7 h-7 rounded-full object-cover border border-gray-200" />
          ) : (
            <div className="shrink-0 w-7 h-7 rounded-full bg-pine-50 border border-gray-200" />
          );
        })()}
        ...
        <input
          ...
          onChange={(e) =>
            setDrafts((s) => ({ ...s, [node.id]: { ...s[node.id], name: e.target.value } }))
          }
        ...
        <input
          type="text"
          value={drafts[node.id]?.photo ?? ""}
          onChange={(e) =>
            setDrafts((s) => ({ ...s, [node.id]: { ...s[node.id], photo: e.target.value } }))
          }
