...
function buildEmbedFromQuery(query) {
  // เติม z= (ระดับซูม) เริ่มต้นไว้เสมอ กัน Google Maps ซูมออกไปเห็นทั้งโลกเวลาที่อยู่/ชื่อสถานที่
  // มีวงเล็บ/รายละเอียดเยอะจนไม่ match แบบเป๊ะ ๆ (เช่น "(บริเวณตรงข้าม...)") — z=16 คือระดับที่เห็น
  // ระดับถนน/ตึกใกล้เคียงชัดเจน ถ้า Google ยัง match สถานที่ได้แม่นอยู่แล้วก็จะซูมเข้าตามปกติ
...
export function toEmbeddableMapsUrl(input) {
  ...
  // แอดมินบางคนกด "แชร์ > ฝังแผนที่" ใน Google Maps แล้วก๊อปทั้งแท็ก
  // <iframe src="https://www.google.com/maps/embed?pb=..." ...></iframe> มาวางทั้งดุ้น
  // (ไม่ใช่แค่ลิงก์เฉย ๆ) — ดึงเอาแค่ค่าใน src="..." ออกมาก่อน แล้วค่อยประมวลผลเป็นลิงก์ต่อ
